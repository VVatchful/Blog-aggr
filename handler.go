...
func handlerRegister(s *state, cmd command) error {
	...
	// Query the database to check if a user with the same name exists
	_, err := s.db.GetUser(context.Background(), name)
	if err != nil && err != sql.ErrNoRows {
		// If an error occurs (other than no rows found), return it
		return fmt.Errorf("error checking for existing user: %w", err)
	}

	if err == nil {
		// If no error and a user was found, return an error for duplicate
